...
def get_site_timeline(site_id: str, settings: Settings | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ensure_schema(conn)
        rows = conn.execute(
            "SELECT e.id, e.document_id, e.site_id, e.iso_date, e.role, e.label, "
            "d.title, d.firm, d.doc_type, d.project_id, d.source_path "
            "FROM events e LEFT JOIN documents d ON d.document_id = e.document_id "
            "WHERE e.site_id = ? ORDER BY e.iso_date, e.id",
            (site_id,),
        ).fetchall()
