...
1. **Carte** — `GET /sites`, un pin par site avec `lat`/`lon`. Clic → sélection du site. Clé `VITE_GOOGLE_MAPS_API_KEY` (rebuild Compose si elle change). Sans clé : liste cliquable.
